    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <table id="example1" class="table table-bordered table-striped">
          ...
          <tbody>
            <?php 
              $query = mysqli_query($koneksi, "SELECT barang.`id`,barang.`nama`, supplier.`nama` AS supplier, stok.`stok_masuk`, stok.`id`, stok.`stok_sekarang`,stok.`tanggal`,stok.`kadaluarsa`,stok.`harga`
              FROM stok
              JOIN barang
              ON barang.`id`=stok.`id_barang`
              JOIN supplier
              ON supplier.`id_supplier`=stok.`id_supplier`
              where stok.stok_sekarang >= 1 and kadaluarsa > now()");
