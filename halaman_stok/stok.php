...
elseif($no1>0){
  echo "<script>swal('Ada $no1 Barang Stoknya Menipis', '', 'info');</script>";
} 
?>
<?php
$date = date('Y-m-d');

$kdla = $koneksi->query("SELECT barang.`nama`, stok.`stok_masuk`, stok.`id`, stok.`stok_sekarang`,stok.`tanggal`,stok.`kadaluarsa`,stok.`harga`
FROM stok
JOIN barang
ON barang.`id`=stok.`id_barang`
...
<div class="row">
  <section class="content-header">
    ...
    <a>     </a>
    <a data-toggle="modal" href="#kdl" class="btn btn-info btn-lg"><i class="fa fa-clock-o"></i> Stok Barang Akan Kadaluarsa</a>
    <a>     </a>
    <a data-toggle="modal" href="#kdla" class="btn btn-warning btn-lg"><i class="fa fa-clock-o"></i> Stok Barang Kadaluarsa</a>
...
<div class="box box-primary">
  ...
  <div class="box-body">
    ...
    <a>  </a>
        <a href="halaman_stok/print2.php" class="btn btn-danger"><i class="fa fa-print"></i> Export PDF</a>
    ...
    <br><br>
      ...
      <tbody>
        ...
        include 'data/function_dateindo.php';
              $query = mysqli_query($koneksi, "SELECT barang.`nama`, supplier.`nama` AS supplier, stok.`stok_masuk`, stok.`id`, stok.`stok_sekarang`,stok.`tanggal`,stok.`kadaluarsa`,stok.`harga`
              FROM stok
              JOIN barang
              ON barang.`id`=stok.`id_barang`
              JOIN supplier
